...
from .util import TASKNAME2TASK, configure_gpu, set_seed, get_weights, get_weights_per_y, get_weights_v2, get_weights_v1
...
class RvSDataset(Dataset):
    ...
    def __init__(self, x, y, w=None, device=None, mode='train', augment=False):
        self.device = device
        self.mode = mode
        self.x = x
        self.y = y
    ...
    def __getitem__(self, idx):
        x = torch.tensor(self.x[idx]).to(torch.float32)
        ...
        if self.device is not None:
            ...
            if w is not None:
                w = w.to(self.device)
        '''
        # print(x.type(), y.type(), w.type() if w is not None else None)
        if w is None:
            return x, y
        else:
            return x, y, w
...
def split_dataset(task, X, y, val_frac=None, device=None, temp=None,augment=False):


    # X, y = task.get_N_non_dominated_solutions(
    #     N=int(task.x.shape[0] * data_preserved_ratio),
    ...
    if not task.is_discrete:
        ...
        #     x = x.reshape(-1, task.x_test.shape[-1])
        #     y = y.reshape(-1, task.y_test.shape[-1])
        # else:
    ...
    # w = get_weights(y[:, 1:], temp=temp)

    w = get_weights_v1(y)
    # w = get_weights_v2(np.sum(y, axis=1), temp=temp)
    # w = get_weights_per_y(y, temp=temp)
    # print(w)
    if augment:
...
class RvSDataModule(pl.LightningDataModule):
    ...
    def __init__(self, task, X, y, batch_size, num_workers, val_frac, device, temp, data_preserved_ratio=1.0, augment=False):
        super().__init__()
        self.task = task
        self.X = X
        self.y = y
        ...
        self.weight_dataset = None
        self.temp = temp
        # self.data_preserved_ratio = data_preserved_ratio
        self.augment = augment

    def setup(self, stage=None):
        self.train_dataset, self.val_dataset = split_dataset(
            self.task, self.X, self.y, self.val_frac, self.device, self.temp,augment=self.augment)
...
def run_evaluate(
    ...
    for lmbd in lmbds:
        if not task.is_discrete:
            # if task.dataset_name == 'mo_hopper_v2' or task.dataset_name == 'mo_swimmer_v2':
            #     x_0 = torch.randn(num_samples, task.x_test.shape[-1],
