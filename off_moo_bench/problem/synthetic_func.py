...
class TE1(SyntheticProblem):
    def __init__(self, n_dim=1):
        super().__init__(
            name=self.__class__.__name__,
            n_dim=n_dim,
            n_obj=1,
            lbound=torch.tensor([0.01]).float(),  # 避免除以0，设置较小的下界
            ubound=torch.ones(n_dim).float() * 100.0,  # 设置上界为10
            nadir_point=[10.0],  # 根据需求调整
            ideal_point=[10.0],  # 根据需求调整
        )
        self.func = self.get_func()
