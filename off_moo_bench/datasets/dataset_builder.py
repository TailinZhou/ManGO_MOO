...
class DatasetBuilder(abc.ABC):
    ...
    def get_xy_min_max(self):
        ...
        if self.pf is not None and 'zdt' in self.name :
            y_min = np.minimum(y_min, self.pf.min(axis=0))
            y_max = np.maximum(y_max, self.pf.max(axis=0))
    ...
    def subsample(self, max_samples=None, 
                  max_percentile=1.0, min_percentile=0.0):
        ...
        if min_percentile != 0.0 or max_percentile != 1.0:
            
            best_min_percentile_indices = get_N_nondominated_indices(
                Y=y,
                num_ret= int(y.shape[0] * min_percentile),
                fronts=fronts
            )
            best_max_percentile_indices = get_N_nondominated_indices(
                Y=y,
                num_ret= int(y.shape[0] * max_percentile),
                fronts=fronts
            )
    ...
    def normalize_x(self, x, normalization_method="None"):
        ...
        if self.x_normalize_method == "z-score" or normalization_method == "z-score":
            return (x - self.x_mean) / self.x_standard_dev
        elif self.x_normalize_method == "min-max" or normalization_method == "min-max":
            return (x - self.x_min) / (self.x_max - self.x_min)
        else:
            raise NotImplementedError

    def normalize_y(self, y, normalization_method="None"):
        ...
        # normalize the prediction values
        if self.y_normalize_method == "z-score" or normalization_method == "z-score":
            return (y - self.y_mean) / self.y_standard_dev
        elif self.y_normalize_method == "min-max" or normalization_method == "min-max":
